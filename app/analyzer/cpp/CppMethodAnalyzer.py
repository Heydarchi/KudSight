...
class CppMethodAnalyzer(AbstractAnalyzer):
    ...
    def analyze(self, filePath, lang=None, classStr=None):
        ...
        while current_pos < len(full_content_for_boundaries):
            next_access_match = re.search(self.access_pattern, full_content_for_boundaries[current_pos:], re.MULTILINE)
            next_access_pos = current_pos + next_access_match.start() if next_access_match else len(full_content_for_boundaries)
            ...
            while inner_pos < len(search_block):
                ...
                method_header = full_content_for_boundaries[abs_match_start:abs_match_end]
                
                if "getData" in method_header:
                     print(f"DEBUG: Regex matched potential getData header: '{method_header.strip()}'")

                methodInfo = self.extractMethodInfo(method_header, match, current_access)
                ...
                if methodInfo:
                    ...
                    if header_strip.endswith(';'):
                        ...
                        if methodInfo.name == "getData":
                             print(f"DEBUG: Searching boundary for getData starting at index {boundary_search_start}")
                        
                        boundary = AnalyzerHelper().findMethodBoundary(full_content_for_boundaries[boundary_search_start:])
                        ...
                        if boundary > 0:
                            methods.append(methodInfo)
                            method_body_end_in_block = inner_pos + match.start() + boundary
                            inner_pos = method_body_end_in_block
                        else:
                            print(f"WARN: No boundary found for potential method definition '{methodInfo.name}'. Adding based on header. Header: {header_strip}")
                            methods.append(methodInfo)
                            inner_pos += match.end()
                else:
                    inner_pos += match.end()
            ...
            if next_access_match:
                ...
                current_pos = line_end + 1 if line_end != -1 else len(full_content_for_boundaries)
    ...
    def extractMethodInfo(self, inputString, match, current_access):
        ...
        if not is_destructor:
             if cleaned_group1 and cleaned_group1.split('<')[0] == method_name.split('<')[0]:
                  is_constructor = True
             elif not cleaned_group1 and not method_name.startswith('~'):
                  is_constructor = True

        if is_destructor:
            methodInfo.name = method_name if method_name.startswith('~') else cleaned_group1
