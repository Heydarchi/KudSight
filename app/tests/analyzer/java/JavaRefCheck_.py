...
class TestJavaRefCheck(unittest.TestCase):
    def setUp(self):
        # Create a temporary directory to store generated files
        self.temp_dir = tempfile.mkdtemp()

        # Define paths
        self.test_java_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(__file__))),
            "test_files",
            "java",
        )
        self.ref_json_path = os.path.join(self.test_java_path, "ref.json")
        self.ref_puml_path = os.path.join(self.test_java_path, "ref.puml")
    ...
    def test_java_analysis_against_ref(self):
        # Analyze Java files
        ...
        # Now analyze
        file_analyzer.analyze(self.test_java_path)
        ...
        key_classes = [
            "com.android.systemui.car.CarDeviceProvisionedController",
            "com.android.systemui.car.CarDeviceProvisionedControllerImpl",
            "com.kudsight.samples.DataProcessor<R>",
            "com.kudsight.samples.StringProcessor",
        ]
        ...
        self.assertIn(
            "..|>", generated_puml, "Missing implementation relationship in PUML"
        )
        self.assertIn("..>", generated_puml, "Missing dependency relationship in PUML")

        # Verify generic types handling with angle brackets in the PUML
        self.assertIn("<R>", generated_puml, "Missing generic type parameter in PUML")
